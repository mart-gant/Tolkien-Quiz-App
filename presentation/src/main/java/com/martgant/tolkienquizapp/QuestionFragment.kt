...
class QuestionFragment : BaseFragment() {
   ...
   private fun refresh(){
       ...
       buttons.forEachIndexed {index, button -> bindButton(button, answers[index])
   }
   }
       private fun bindButton(button: Button, answer: QuestionProps.Answer){
           button.text = context?.getSring(answer.answerRes)
    ...
    private fun getQuestionProps(): QuestionProps{
        ...
        val incorrectAnswer = QuestionProps.Answer (
            answerRes = getStringByName(ANSWER_INCORRECT_FORMAT.format(questionNumber))
                    isCorrect = false
        )
        val questionRes = getStringByName(ANSWER_CORRECT_FORMAT.format(questionNumber))
